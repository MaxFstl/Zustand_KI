...
class Square:
    ...
    def drawRect(self):
        ...
        text = self.font.render(
            self.policy + " , " + str(int(self.value)), True, (0, 0, 0)
        )
        ...
        if self.player:
            pg.draw.rect(
                self.surface,
                "yellow",
                pg.Rect(self.x, self.y, self.width, self.width),
                int(self.width / 2),
            )
            return
        ...
        if self.lose:
            pg.draw.rect(
                self.surface, "red", pg.Rect(self.x, self.y, self.width, self.width), 1
            )
            return

        pg.draw.rect(
            self.surface, self.color, pg.Rect(self.x, self.y, self.width, self.width), 1
        )
...
class Agent:
    ...
    def step(self):
        ...
        if (
            e > self.exploration
            and self.generation < pow(boardlen, 4)
            or self.currSquare.policy == "N"
        ):
    ...
    def movePlayer(self, xDir: int, yDir: int):
        ...
        if yDir == 0:
            ...
            s = self.currSquare
            self.currSquare = self.board[self.currSquare.xPosOnBoard + xDir][
                self.currSquare.yPosOnBoard
            ]
            self.visitedSquares.append(self.currSquare)
            self.currSquare.player = True
        ...
        if xDir == 0:
            ...
            self.currSquare.player = False
            s = self.currSquare
            self.currSquare = self.board[self.currSquare.xPosOnBoard][
                self.currSquare.yPosOnBoard + yDir
            ]
            self.visitedSquares.append(self.currSquare)
    ...
    def notifyWin(self):
        ...
        self.currSquare.player = False
        self.placeAgent(
            random.randint(0, boardlen - 1), random.randint(0, boardlen - 1)
        )
        self.generation += 1
